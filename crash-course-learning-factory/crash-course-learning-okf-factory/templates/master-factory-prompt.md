...
6. 每次学习后必须更新 state/ 目录、sessions/ 记录和 score-history。
7. 如果我答得不好，请改写后续计划，不要机械推进。
8. 生成后必须运行质检：先检查文件结构，再检查内容是否课程专属、无占位符、Day 1 可直接执行、测验和 mock exam 可考试。
9. 如果质检失败，请先根据 quality-report 修订失败文件，再重新质检；不要把只存在文件但内容空泛的 OKF 标为通过。
10. 输出 zip 包，并告诉我入口文件、Day 1 入口、状态保存位置、恢复学习方法、structural validation、quality gate 和 repair status。
```
