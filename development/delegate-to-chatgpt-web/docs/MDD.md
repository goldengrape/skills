...
| MDD-MOD-004 | Browser Exchange Adapter | ADD-DP-003, ADD-DP-004 | 取得目标标签页，上传附件，核对并发送一次任务。 | 在 Python 中控制浏览器。 |
| MDD-MOD-005 | Response Monitor | ADD-DP-005 | 从页面证据产生进行中、完成、需要输入或失败结果。 | 下载和解压文件。 |
| MDD-MOD-006 | Download Resolver | ADD-DP-006 | 记录下载前后文件快照，结合下载事件确定结果文件。 | 判断 ZIP 内容是否安全。 |
...
| MDD-API-004 | 附件文件存在；尚无成功发送收据。 | 页面附件匹配且消息包含 task_id；产生一次发送收据。 | 未核对附件不得发送。 | 返回具体阶段；不得盲目重复点击。 |
| MDD-API-005 | 任务已发送。 | 只返回一个可解释状态和对应页面证据。 | 仅“已完成”文字不足以判定成功。 | 超时返回可恢复状态。 |
| MDD-API-006 | outcome 指向当前 task_id 的结果控件。 | 下载事件和本地新文件同时成立；结果复制到任务目录。 | 不覆盖已有结果。 | 区分事件超时和文件未找到。 |
...
4. 每个副作用前后写入状态事件。
5. 按 `references/task-protocol-and-schemas.md` 发送和解析任务协议。
6. 下载后调用结果验证脚本。
