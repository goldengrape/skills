...
5. creates the teacher runtime layer (`teacher/`) and checks prompt visibility, score type, time policy, interest-led branches, and engagement rules.
...
- teaching runtime files;
- prompt-visibility leakage in student-visible prompts;
- score type and prompt-visibility fields;
- soft/strict time policy;
- interest and engagement rules.

## `tools/lint_prompt_visibility.py`

Checks student-visible plan and quiz prompts for pre-answer leakage of hidden answer elements, and verifies the teacher runtime layer.

```bash
python tools/lint_prompt_visibility.py ./out/course-okf-macroeconomics-pass
```
...
validation_result:
  ...
  # quality_gate includes content quality and teaching runtime quality
  structural: ...
  quality_gate: ...
```
