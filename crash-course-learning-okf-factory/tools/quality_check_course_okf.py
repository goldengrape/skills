...
def quality_check(root: Path, course_name: Optional[str] = None, days_available: Optional[int] = None) -> Dict[str, Any]:
    ...
    # 6. Teaching runtime quality: no hidden answer leakage, teacher notebook, score type, soft/strict time policy, interest rules.
    teaching_runtime = lint_prompt_visibility(root)
    if not teaching_runtime["passed"]:
        for failure in teaching_runtime["failures"]:
            failures.append({
                "code": failure["code"],
                "path": failure["path"],
                "message": failure["message"],
            })
            repair_actions.append(f"Fix teaching runtime issue in `{failure['path']}`: {failure['message']}")
    ...
    hard_gate_failures = [f for f in failures if f["code"] in {
        ...
        "missing_teaching_runtime_file",
        "prompt_visibility_leak",
        "teacher_notebook_schema_missing",
        "visibility_rules_incomplete",
        "time_policy_missing_soft_strict",
